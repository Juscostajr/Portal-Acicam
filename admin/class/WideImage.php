 ...
 * @copyright 2007-2011
  ...
  **/
	...
	class WideImage
	{
		...
		static function proporcao($largura,$altura,$lim_largura,$lim_altura)
		{
			if ($altura >= $largura) {
				if ($altura > $lim_altura) {
					return array(($largura * $lim_altura) / $largura, $lim_altura);
				} else {
					return array($largura, $altura);
				}
			} else {
				if ($largura > $lim_largura) {
					return array($lim_largura, ($altura * $lim_largura) / $largura);
				} else {
					return array($largura, $altura);
				}
			}
